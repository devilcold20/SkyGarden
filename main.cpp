...
Rect	rct_bg = {0, 1280, 720, 0}, //Màn hình menu
		...
		rct_bagScreen = {0, 1280, 720, 0},  //Bg Màn hình shop
		rct_missionScreen = {400, 506 + 400, 604 + 67, 67},  //Bg Màn hình mission
...
//Khai báo hình ảnh
Image	img_bg, img_bg2, img_helpScreen, img_exit_YN, img_shopScreen,img_missionScreen,img_bagScreen,//bg
		//buttton
...
//Khai báo hàm
void init();
void initStore(); // init shop và bag
...
void screenBag(); //Hiển thị màn hình Bag
void screenMission(); //
void screenBag(); //
...
void mouseClickInBag(int button , int state, int x, int y); // click trong bag
...
void init_inGame() {
 //Load bg
  ...
  Load_Texture_Swap( & img_bagScreen, "Images/bg4-Bag.png");
...
bool checkNewGame = false; // xử lý countinue
bool checkResetGame = false;// xử lý reset new game
//màn hình newgame
void screenNewGame() {
...
void clickMenuIntro(int button, int state, int x, int y) {
  ...
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 150 && x <= 400 && y >= 475 && y <= 550 && checkNewGame == false)
  {
  ...
  //btn continues rct_btn_continue={150, 250 + 150, 75 + 575, 575},
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 150 && x <= 400 && y >= 575 && y <= 575 + 75 && checkNewGame == true)
  {
	  cout <<"Qua Countinue thanh cong\n";
	  glutDisplayFunc(screenNewGame);
  }
  glutPostRedisplay();
}
...
void clickBack(int button, int state, int x, int y) {
  ...
  // go to bag
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 1170 && x <= 85 + 1170 && y >= 280 && y <= 73 + 280 && screen == 0) {
    cout << "Qua Bag thanh cong\n";
	glutDisplayFunc(screenBag);
  }
  // goto mission
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 1170 && x <= 76 + 1170 && y >= 190 && y <= 79 + 190 && screen == 0)
  {
	  cout << "show màn mission\n";
	  glutDisplayFunc(screenMission);
  }
  // back new game tu mission
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 850 && x <= 895 && y >= 85 && y <= 138 && screen == 2){
	  cout << "close mission\n";
	  glutDisplayFunc(screenNewGame);
  }
  glutPostRedisplay();
...
// hàm xử lý sự kiện trong túi
void mouseClickInBag(int button , int state, int x, int y)
{
	  //back về screen Newgame
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 20 && x <= 70 && y >= 20 && y <= 95 && screen == 1) {
		cout << "Back to NewGame\n";
